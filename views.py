...
from dao import UsuarioDao, CategoriaDao,ItemDao
...
@server.route('/deletar_categoria/<int:id>')
...
@server.route('/lista_itens')
def lista_itens():
    #id = session['id_propriedade']
    lista = item_dao.listar()
    tit_lista = 'Manutenção de Itens do Museu Virtual'
    return render_template('lista_itens.html', titulo=tit_lista, lista=lista)

@server.route('/novo_item')
def novo_item():
    if 'usuario_logado' not in session or session['usuario_logado'] == None:
        return redirect(url_for('login', proxima=url_for('novo_item')))
    return render_template('novo_item.html',
                           titulo='Cadastro de Itens do Museu Virtual')
'''
@server.route('/criar_categoria', methods=['POST',])
def criar_categoria():
    desc = request.form['descCategoria']
...
def atualizar_categoria():
    ...
    return redirect(url_for('lista_categorias'))
'''
@server.route('/deletar_item/<int:id>')
def deletar_item(id):
    item_dao.deletar(id)
