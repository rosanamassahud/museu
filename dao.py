...
SQL_SELECT_ITEM = "SELECT codigoItem, descricaoItem, " \
                  ...
                  "DATE_FORMAT(dataCadastro, '%d/%m/%Y %H:%i:%s') as dataCadastro, tipoItem AS tipoItem, nomeArquivo, extensaoArquivo, " \
                  ...
                  "ORDER BY item.dataCadastro desc, item.dataAquisicao ASC, c.ordemPainel asc"
...
class ItemDao:
    ...
    def deletar(self, id):
        try:
            sql = SQL_DELETE_ITEM.format(id)
            self.__db.connection.cursor().execute(sql)
            self.__db.connection.commit()
...
def traduz_itens(categorias):
    def cria_item_com_tupla(tupla):
        return Item(tupla[1], tupla[2], tupla[3], tupla[4],tupla[5], tupla[6], Usuario(tupla[7], tupla[8], tupla[9]), Categoria(tupla[10], tupla[11], id=tupla[12]),codigo=tupla[0])
